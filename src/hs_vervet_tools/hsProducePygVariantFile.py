...
class CalculateStatsForSubPop(AbstractVervetMapper):
	...
	def selectUCLAids(self,metadata):#cond_dict
		#just take pyg individuals
		ind_ls=[]
		for ind in xrange(len(metadata[2])):
			if metadata[2][ind]=='460674':
				ind_ls.append(ind)
		uclaid_ls=[metadata[1][i] for i in ind_ls]
		return uclaid_ls	
	...
	def appendTransPLINK(self,datastruct):
		...
		#import pdb
		data=datastruct.data
		tpedFname='/media/Data/Akademisches_data/vervetpopgen/AfricaExtractedData/Meth40AllCont1.tped'
	...
	def saveVCF_ids(self,vcfid_ls,indx):
		outFnameID='/home/GMI/hannes.svardal/Desktop/lab/vervetpopgen/AfricaExtractedData/Meth40AllCont_VCFind_ls.tsv'
		writerID =csv.writer(open(outFnameID, 'w'), delimiter='\t')
		writerID.writerow(vcfid_ls)
		writerID.writerow(indx)
	...
	def append01file(self,datastruct):
		...
		Fname01='/media/Data/Akademisches_data/vervetpopgen/AfricaExtractedData/Meth40AllCont_each1000snp_01_mat.tsv'
	...
	def freqFilter(self,datastruct,cutoff):
		aaf=self.calculateAAF(datastruct.data)
		abovecutoffpos=np.nonzero((aaf>cutoff)*(aaf<1-cutoff))
		abovecutoffpos=abovecutoffpos[0]
		#print abovecutoffpos
		#print abovecutoffpos.shape
		#print aaf.shape
		datastructCUT=hsContigDataStruct(ind_id_ls=datastruct.ind_id_ls, chrom_ls=datastruct.chrom_ls[abovecutoffpos],
									ref_ls=datastruct.ref_ls[abovecutoffpos], snp_pos_ls=datastruct.snp_pos_ls[abovecutoffpos],
									alt_ls=datastruct.alt_ls[abovecutoffpos], data=datastruct.data[abovecutoffpos,:])
		return datastructCUT	
		
	def calculateMAF(self,aaflist):
		pass
	
	def createPygSNPfile(self):
		Fname01='/home/GMI/hannes.svardal/Desktop/lab/vervetpopgen/AfricaExtractedData/Meth40AllCont_PygSNPs.tsv'
		writer01mat=csv.writer(open(Fname01, 'w'), delimiter='\t')
		firstrow=['contig','position','ref','alt','alt_count']
		writer01mat.writerow(firstrow)
		del writer01mat
	
	def appendPygSNPfile(self,datastruct,aaf_list):
		data=datastruct.data
		#write 0 1 matrix (e.g. for t-sne)
		Fname01='/home/GMI/hannes.svardal/Desktop/lab/vervetpopgen/AfricaExtractedData/Meth40AllCont_PygSNPs.tsv'
		writer01mat=csv.writer(open(Fname01, 'a'), delimiter='\t')
		#set missing data to reference allele
		mat=np.column_stack((datastruct.chrom_ls,datastruct.snp_pos_ls,datastruct.ref_ls,datastruct.alt_ls,(aaf_list*2*data.shape[1]).astype(int)))
		writer01mat.writerows(mat)		
		#sys.stderr.write("%s loci outputted.\n"%(counter))
		del writer01mat
	
	def createContigFnameFile(self):
		
		#create filename list
		Fname="/home/GMI/hannes.svardal/Desktop/lab/vervetpopgen/AfricaExtractedData/Method39ContigNames.tsv"
		...
		outFname="/home/GMI/hannes.svardal/Desktop/lab/vervetpopgen/AfricaExtractedData/Method39ContigFNameList.tsv"
		writerID =csv.writer(open(outFname, 'w'), delimiter='\t')
		writerID.writerow(genotypeFname_ls)
		del writerID
		#self.loadMetadataMat("/home/GMI/hannes.svardal/Desktop/lab/vervetpopgen/AfricaExtractedData/AfriCaribMetaData.tsv")
		#idls=self.selectUCLAids(self.metadata)	
		
		
		#(indx,ind_id_ls)=self.getVCFInd(idls)
		#self.saveVCF_ids(ind_id_ls,indx,genotypeFname_ls)
	...
	def run(self):
		...
		FnameID='/home/GMI/hannes.svardal/Desktop/lab/vervetpopgen/AfricaExtractedData/Meth40AllCont_VCFind_ls.tsv'
		lines=open(FnameID).readlines()
		VCFID_ls=lines[0].split()
		VCFind_ls=map(int,lines[1].split())
		contigFname="/home/GMI/hannes.svardal/Desktop/lab/vervetpopgen/AfricaExtractedData/Method39ContigFNameList.tsv"
		contigFname_ls=open(contigFname).readline().split()
		
		#raw_input("Press Enter to continue...")
		#self.saveVCF_ids(ind_id_ls,indx)
		#tok=1
		
		self.createPygSNPfile()
		
		singleton_ls=[]
		retainedSNP_ls=[] 
		#for contig in contiglist:
		for i in xrange(len(contiglist)):
			self.chromosome=contiglist[i]		
			datastruct=self.selectSubPopNoDB(VCFind_ls,VCFID_ls,contigFname_ls[i])
			datastructNF=self.nonFixedDataList(datastruct)
			#print datastruct.data.shape
			#print datastructNF.data.shape
			#print 1/(2*datastructNF.data.shape[1])
			cutoff=1./(2*datastructNF.data.shape[1])
			print cutoff
			datastructCUT=self.freqFilter(datastructNF,cutoff)
			singleton_ls.append(datastructNF.data.shape[0]-datastructCUT.data.shape[0])
			retainedSNP_ls.append(datastructCUT.data.shape[0])
			aaf_ls=self.calculateAAF(datastructCUT.data)			
			self.appendPygSNPfile(datastructCUT,aaf_ls)
		...
		print np.array(singleton_ls).sum()
		print np.array(retainedSNP_ls).sum()
		FnameSING='/home/GMI/hannes.svardal/Desktop/lab/vervetpopgen/AfricaExtractedData/Meth40AllCont_PygSNPs_retainedSNPs_ans_singletons.tsv'
		writer01mat=csv.writer(open(FnameSING, 'w'), delimiter='\t')
		contiglist.reverse()
		contiglist.append('Contig')
		contiglist.reverse()
		retainedSNP_ls.reverse()
		retainedSNP_ls.append('SNPs_no_sing')
		retainedSNP_ls.reverse()
		singleton_ls.reverse()
		singleton_ls.append('singletons')
		singleton_ls.reverse()
		writer01mat.writerow(contiglist)
		writer01mat.writerow(retainedSNP_ls)
		writer01mat.writerow(singleton_ls)
		del writer01mat
