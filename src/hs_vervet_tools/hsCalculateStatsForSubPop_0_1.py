...
class CalculateStatsForSubPop(AbstractVervetMapper):
	...
	def produceTFAM(self,datastruct):
		tfamFname='/media/Data/Akademisches_data/vervetpopgen/AfricaExtractedData/Meth27AllCont1.tfam'
		#writer = csv.writer(open(self.outputFname, 'w'), delimiter='\t')
	...
	def appendTransPLINK(self,datastruct):
		#---plink  file---
		data=datastruct.data
		tpedFname='/media/Data/Akademisches_data/vervetpopgen/AfricaExtractedData/Meth27AllCont1.tped'
	...
	def run(self):
		...
		ind=84
		for contig in contiglist[84:]:
			self.chromosome=contig
			datastruct=self.selectSubPop(idls)
			...
			ind=ind+1
			print ind,"), plink tfam Meth27AllCont1.tped appended with ", len(datastruct.snp_pos_ls), "snps from", contig 
